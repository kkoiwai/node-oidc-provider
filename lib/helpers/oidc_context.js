...
module.exports = function getContext(provider) {
  ...
  class OIDCContext extends events.EventEmitter {
    ...
    #requestParamVerifiedClaims = null;
        ...
        if (userinfo) {
          Object.entries(userinfo).forEach(([claim, value]) => {
            if (claims.has(claim) && (value === null || isPlainObject(value))) {
              if (!verifiedClaims.enabled || claim !== "verified_claims") {
    ...
    get requestParamVerifiedClaims() {
      if (this.#requestParamVerifiedClaims) {
        return this.#requestParamVerifiedClaims;
      }
      const requestParamVerifiedClaims = { userinfo: null, idToken: null };
      if (verifiedClaims.enabled) {
        if (this.params.claims) {
          const {
            userinfo, id_token: idToken,
          } = JSON.parse(this.params.claims);

          // TODO: should check with verifiedClaims.trust_frameworks_supported, evidence_supported etc..

          if (userinfo) {
            Object.entries(userinfo).forEach(([claim, value]) => {
              if (claim == "verified_claims") {
                requestParamVerifiedClaims.userinfo = value;
              }
            });
          }
          ...
          if (idToken) {
            Object.entries(idToken).forEach(([claim, value]) => {
              if (claim == "verified_claims") {
                requestParamVerifiedClaims.idToken = value;
              }
            });
          }
        }
      ...
      this.#requestParamVerifiedClaims = requestParamVerifiedClaims;

      return requestParamVerifiedClaims;
    }
    ...
    getAccessToken({
      acceptDPoP = false, acceptQueryParam = !fapi.enabled && acceptQueryParamAccessTokens,
    } = {}) {
      if (this.#accessToken) {
