...
const missingOIDCVerifiedClaims = Symbol();
...
module.exports = () => new Prompt(
  ...
  new Check('op_verified_claims_missing', 'requested verified claims not granted', (ctx) => {
    // TODO: for now, always return REQUEST_PROMPT unless the granted vc is the same
    const { oidc } = ctx;

    if (JSON.stringify(oidc.grant.getOIDCVerifiedClaimsEncountered()) !== JSON.stringify(oidc.requestParamVerifiedClaims) &&
      (oidc.requestParamVerifiedClaims.idToken != null || oidc.requestParamVerifiedClaims.userinfo != null)) {
      ctx.oidc[missingOIDCVerifiedClaims] = oidc.requestParamVerifiedClaims;
      return Check.REQUEST_PROMPT;
    }

    return Check.NO_NEED_TO_PROMPT;
  }, ({ oidc }) => ({ missingOIDCVerifiedClaims: oidc[missingOIDCVerifiedClaims] })),
