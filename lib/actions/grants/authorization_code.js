...
module.exports.handler = async function authorizationCodeHandler(ctx, next) {
  ...
  console.log("[DEBUG] AccessToken saved at authorization_code.js: " + JSON.stringify(at, null, 2));

  console.log("[DEBUG] ctx: " + JSON.stringify(ctx, null, 2));
  console.log("[DEBUG] ctx.oidc.client: " + JSON.stringify(ctx.oidc.client, null, 2));
  console.log("[DEBUG] code: " + JSON.stringify(code, null, 2));
  ...
  if (await issueRefreshToken(ctx, ctx.oidc.client, code)) {
    ...
    console.log("[DEBUG] RefreshToken saving at authorization_code.js: " + JSON.stringify(rt, null, 2));
    refreshToken = await rt.save();
    console.log("[DEBUG] RefreshToken saved at authorization_code.js: " + JSON.stringify(rt, null, 2));
  }

  let idToken;
  if (code.scopes.has('openid')) {
    const claims = filterClaims(code.claims, 'id_token', grant, ctx);
